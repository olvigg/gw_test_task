...
function App() {
  ...
  const changeSort = (field) => {
    const newSorting = {
      field,
      dir: field === sort.field && sort.dir === "asc" ? "desc" : "asc",
    };
    Cookies.set("default-sorting", JSON.stringify(newSorting), {
      path: "/",
      domain: cookieDomain,
    });
    setSort(newSorting);
  ...
  const getSortIcon = (field) => {
    if (sort.field === field) {
      return sort.dir === "asc" ? ( //may be here will be better 'desc', for UX
      ...
      ? orderBy(
          ...
          [
            (it) => {
              switch (sort.field) {
                case "name":
                  return it.folderName.toLowerCase();
                case "size":
                  return it.size;
                case "mtime":
                  return it.mtimes[sort.dir === "asc" ? "max" : "min"];
                default:
                  return;
              }
            },
  ...
  useLayoutEffect(() => {
    fetch(
      ...
      .then(async (res) => {
        ...
        setSort(
          defaultSortCookie && isJson(defaultSortCookie)
  ...
  return (
    <div className="app">
      ...
      <div className="content">
        {!loading && sorted && (
          <>
            <div className="sort_wrap">
              ...
              {map(sortableFields, (it, i) => {
                return (
                  <span
                    key={`sf${i}`}
                    className={`${it.field === sort.field ? "active" : ""}`}
            ...
            <div className="tree">
              {map(sorted, (folder, i) => {
                return (
                  <div className="line folder" key={`lfolder${i}`}>
                    <FontAwesomeIcon
                      icon="folder-open"
                    ...
                    {map(folder.files, (file, j) => {
                      return (
                        <div className="line file" key={`lfile${j}`}>
                          {file.icon}
        ...
        {loading && (
          <div className="msg loading">Wait, please. Data is loading...</div>
        )}
